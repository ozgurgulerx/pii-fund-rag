      ...
      async start(controller) {
        try {
          ...
          for (let i = 0; i < words.length; i++) {
            ...
            // Small delay for streaming effect (reduced from 15ms for faster response)
          ...
          if (data.citations && data.citations.length > 0) {
            const formattedCitations: Citation[] = data.citations.map(
              (c: { source_type: string; identifier: string; title: string; content_preview: string; score: number }, idx: number) => ({
                id: idx + 1,  // Temporary ID, will be reassigned in frontend for proper sequencing
          ...
          controller.enqueue(
            encoder.encode(
              createSSEMessage({
                type: "done",
                isVerified: data.citations && data.citations.length > 0,
              })
            )
          );
